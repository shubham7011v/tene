...
class SignInConfig {
  ...
  static GoogleSignIn getGoogleSignIn() {
    // Server client ID for web (not needed for Android-only apps)
    const webClientId = '148139223467-kvuc5kamhd6are6i2u4ood3gocgufvfc.apps.googleusercontent.com';

    // For Android, signInOption.standard works better than games or web
    return GoogleSignIn(
      // Keep scopes minimal for better success rate
      scopes: ['email'],
      // Use standard sign-in for Android
      signInOption: SignInOption.standard,
      // Only include serverClientId if needed for web or backend verification
      // Commenting out for now as it can cause issues on some Android devices
  ...
  static String formatSignInError(dynamic error) {
    ...
    if (errorMessage.contains('network_error') || errorMessage.contains('failed host lookup')) {
